...
cartRouter.put("/", async (req, res, next) => {
    try {
        const {orderN, state,totalPrice,date,trackingNumber, delivery} = req.body;
        const updatedCart = await updateCart(orderN,state,totalPrice,date,trackingNumber, delivery);
